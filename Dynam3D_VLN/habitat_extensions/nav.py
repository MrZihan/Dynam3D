...
class MoveHighToLowAction(SimulatorTaskAction):
    ...
    def turn(self, angle):
        ...
        if angle == 0:
            self._sim.step_without_obs(HabitatSimActions.TURN_LEFT)
            output = self._sim.step(HabitatSimActions.TURN_RIGHT)
            return output
        ...
        for turn_action in turn_actions:
            output = self._sim.step(turn_action)
            # print(self.cal_heading(self._sim.get_agent_state()))
        return output
    ...
    def step(self, *args: Any, 
        ...
        if not niu1niu:
            ...
            if 0. < distance and distance < foward_unit:
                distance = foward_unit
            ...
            if forward_step > 0:
                ...
                output = self.turn(init_try_angle)
