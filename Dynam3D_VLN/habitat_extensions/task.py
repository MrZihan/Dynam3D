...
class VLNCEDatasetV1(Dataset):
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        if os.path.isdir(dataset_filename):
            self.from_dir(dataset_filename, scenes_dir=config.SCENES_DIR)
        else:
            with gzip.open(
        ...
        if "train" not in dataset_filename:
    ...
    def from_dir(
        self, dir_path: str, scenes_dir: Optional[str] = None
    ) -> None:

        scene_list = os.listdir(dir_path)
        for i in range(len(scene_list)):
            scene_list[i] = dir_path+"/"+scene_list[i]

        seed = int(time.time())
        random.seed(seed)
        
        hm3d_scene_list = [scene for scene in scene_list if "basis" in scene] # HM3D
        mp3d_scene_list = [scene for scene in scene_list if "basis" not in scene] # MP3D

        if random.choices([0,1])[0] == 0:
            scene_list = random.choices(hm3d_scene_list,k=2)
        else:
            scene_list = random.choices(mp3d_scene_list,k=2) # !!!!!!!!!!!!
        print(scene_list)
        for scene_path in scene_list:
            deserialized = json.load(open(scene_path,"r"))
            for episode in deserialized:
                ...
                if scenes_dir is not None:
                    ...
                    if scenes_dir not in episode.scene_id:
...
class RxRVLNCEDatasetV1(Dataset):
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        for role in self.extract_roles_from_config(config):
            if os.path.isdir(config.DATA_PATH.format(split=config.SPLIT, role=role)):
                self.from_dir(config.DATA_PATH.format(split=config.SPLIT, role=role), scenes_dir=config.SCENES_DIR)
            else:
                with gzip.open(
        ...
        if "train" not in config.DATA_PATH.format(split=config.SPLIT, role=role):
            if ALL_SCENES_MASK not in config.CONTENT_SCENES:
                scenes_to_load = set(config.CONTENT_SCENES)
                self.episodes = [
                    episode
