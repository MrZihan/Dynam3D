...
def build_habitat_sim(scan):
    sim = HabitatUtils(f'data/scene_datasets/mp3d/{scan}/{scan}.glb', int(0), int(math.degrees(HFOV)), HEIGHT, WIDTH)
    return sim


def process_features(item_id, episode_id, discrete_data, out_queue):
    sim = build_simulator("connectivity_mp3d")
    ...
    for item in tqdm(discrete_data[item_id:]):
        ...
        for instruction_id in range(len(item['instructions'])):
            episode_id += 1
            item_id += 1
            CE_item = {
                "episode_id":episode_id,
                "trajectory_id":episode_id,
                "scene_id":"mp3d/"+item['scan']+"/"+item['scan']+".glb",
                ...
                "instruction": {
                    "instruction_text": item['instructions'][instruction_id], 
                    "instruction_tokens": item['instr_encodings'][instruction_id]
                },
...
discrete_data = json.load(open("REVERIE_val_unseen_enc.json","r"))
discrete_data_new = {}
...
for key in tqdm(discrete_data):
    ...
    out_queue = Queue()
    process_features(item_id, episode_id, discrete_data[key], out_queue)
    while True:
        print(episode_id)
        if out_queue.empty():
            break
...
json.dump(CE_data, open("reverie_val_unseen_data.json","w"))
json.dump(CE_data_gt, open("reverie_val_unseen_gt.json","w"))
...
discrete_data = json.load(open("REVERIE_test_enc.json","r"))
discrete_data_new = {}
for item in discrete_data:
    scan_id = item['scan']
    if scan_id in discrete_data_new:
...
json.dump(CE_data, open("reverie_test_data.json","w"))
json.dump(CE_data_gt, open("reverie_test_gt.json","w"))