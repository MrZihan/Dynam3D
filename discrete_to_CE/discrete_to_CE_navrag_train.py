...
scene_list = os.listdir('data/scene_datasets/hm3d/train/')
scene_dict = {}
for scene_id in scene_list:
    scene_dict[scene_id.split("-")[-1]] = scene_id
...
def process_features(item_id, episode_id, discrete_data, out_queue):
    ...
    discrete_data = discrete_data[item_id:]
    random.shuffle(discrete_data)
    for item in tqdm(discrete_data):
        scan_id = item['scan']
        if "-" not in scan_id: # MP3D
            continue
        path = item['path']
        reference_path = []
        ...
        geodesic_distance = habitat_sim.sim.geodesic_distance(reference_path[0], reference_path[-1]) # painting,tight
        if geodesic_distance < 100.:
            pass
        else:
            continue
        if item_id > 2000 or len(item['instructions'][0].split(" ")) > 25 or geodesic_distance > 15. or "picture" in item['instructions'][0] or "man" in item['instructions'][0] or "person" in item['instructions'][0] or "ceiling" in item['instructions'][0] or "painting" in item['instructions'][0] or "tight" in item['instructions'][0]:
            continue
...
discrete_data = json.load(open("NAVRAG_train_enc.json","r"))
discrete_data_new = {}
...
key_id = 0
key_id_json = [0]
if os.path.exists("tmp_navrag_CE_data.json") and os.path.exists("tmp_navrag_CE_data_gt.json") and os.path.exists("navrag_key_id.json"):
    CE_data = json.load(open("tmp_navrag_CE_data.json","r"))
    CE_data_gt = json.load(open("tmp_navrag_CE_data_gt.json","r"))
    key_id_json = json.load(open("navrag_key_id.json","r"))
...
for key in tqdm(discrete_data):
    ...
    key_id_json = [key_id]
    if key_id % 25 == 0:
        json.dump(CE_data, open("tmp_navrag_CE_data.json","w"))
        json.dump(CE_data_gt, open("tmp_navrag_CE_data_gt.json","w"))
        json.dump(key_id_json, open("navrag_key_id.json","w"))
        exit()
...
for scene_id in scene_dict:
    json.dump(scene_dict[scene_id], open("navrag_training_data/"+scene_id.split("/")[-1]+".json","w"))

json.dump(CE_data_gt, open("navrag_train_gt.json","w"))